...
class Context(BaseModel):
    """
    Shared context for the Orchestrator Agent.

    Mirrors the other agents so that user/organization and LLM
    configuration are consistent across the platform.
    """
    ...
    organization_name: Literal["bayes", "verst_carbon", "ignis_innovation", "afcen"] = (
        Field(
            default="bayes",
            description=(
                "The organization name. Determines which organization skill to load."
            ),
        )
    )
