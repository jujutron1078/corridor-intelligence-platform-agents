...
def sync(force=False):
    ...
    # Multipart download: 8 MB chunks, 4 concurrent threads
    ...
    # Skip dirs that aren't essential and eat volume space.
    # Remove entries here if the volume is large enough (1 GB+).
    skip_env = os.environ.get("R2_SKIP_DIRS", "connectivity,livestock")
    SKIP_PREFIXES = {f"v1/data/{d.strip()}/" for d in skip_env.split(",") if d.strip()}
        ...
        for page in paginator.paginate(Bucket=bucket, Prefix=prefix):
            for obj in page.get("Contents", []):
                ...
                try:
                    ...
                    downloaded += 1
                    if downloaded % 50 == 0:
                        logger.info("  ... downloaded %d files so far", downloaded)
                except Exception as e:
                    logger.warning("Failed to download %s (%.0f MB): %s", key, obj.get("Size", 0) / 1e6, e)
                    errors += 1

        logger.info("R2 sync complete: %d downloaded, %d skipped (existing), %d errors", downloaded, skipped, errors)
    except Exception as e:
